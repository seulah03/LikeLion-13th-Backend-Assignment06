...
public class JwtTokenProvider {
    ...
    public String generateToken(Member member) {
        ...
        return Jwts.builder()
            ...
            .claim(AUTHORITIES_KEY, member.getRole().toString()) // 권한 정보 설정
            .issuedAt(now)  // 발행 시간
    ...
    public Authentication getAuthentication(String token) {
        ...
        String authority = claims.get(AUTHORITIES_KEY, String.class); // 권한 정보 추출
        if(authority == null){
            throw new BusinessException(ErrorCode.NO_AUTHORIZATION_EXCEPTION, "권한 정보가 없습니다.");
        }

        List<GrantedAuthority> authorities = Arrays.stream(authority.split(","))
            .map(SimpleGrantedAuthority::new) // 권한을 GrantedAuthority로 변환
